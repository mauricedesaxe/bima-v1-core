...
contract BoostCalculator is IBoostCalculator, SystemStart {
     ...
     */
    function getClaimableWithBoost(address claimant, uint256 previousAmount, uint256 totalWeeklyEmissions)
        external
        view
        returns (uint256 maxBoosted, uint256 boosted)
    {
        uint256 week = getWeek();
    ...
    function _getBoostedAmount(uint256 amount, uint256 previousAmount, uint256 totalWeeklyEmissions, uint256 pct)
        internal
        pure
        returns (uint256 adjustedAmount)
    {
