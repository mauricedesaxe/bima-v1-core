...
contract PriceFeed is IPriceFeed, BabelOwnable {
    ...
    function fetchPrice(address _token) public returns (uint256) {
        ...
        (FeedResponse memory currResponse, FeedResponse memory prevResponse, bool updated) =
            _fetchFeedResponses(oracle.chainLinkOracle, priceRecord.roundId);
    ...
    function _fetchFeedResponses(IAggregatorV3Interface oracle, uint80 lastRoundId)
        internal
        view
    ...
    function _fetchCurrentFeedResponse(IAggregatorV3Interface _priceAggregator)
        internal
        view
    ...
    function _fetchPrevFeedResponse(IAggregatorV3Interface _priceAggregator, uint80 _currentRoundId)
        internal
        view
        returns (FeedResponse memory prevResponse)
    {
